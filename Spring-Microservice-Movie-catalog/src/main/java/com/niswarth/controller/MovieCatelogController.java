...
public class MovieCatelogController {
	...
	public List<CatelogItems> getCatelogItems(String userId){
		...
		UserRating ratings = restTemp.getForObject("http://movie-rating-Microservice/ratings/users/"+userId, UserRating.class);
		
		return ratings.getUserRating().stream().map(rating -> {
			
			//for each movie Id,call movie info service and get details
			Movie movie = restTemp.getForObject("http://movie-info-Microservice/movies/"+rating.getMovieId(), Movie.class);
